...
async function runV9ArchiveCycleTest() {
    console.log("--- 🔄 开始：V9 档案循环生命周期测试 (进进出出+中加人+重复入驻) ---");
    ...
    const baseSuffix = `_V9_${Date.now().toString().slice(-4)}`;
    ...
    if (finalWife && finalWife.spouse_id === bInAFinal_C.id) {
        ...
        console.log(`❌ 关系链错误：媳妇的丈夫关系丢失或错误。`);
    }
...
runV9ArchiveCycleTest().catch(err => {
